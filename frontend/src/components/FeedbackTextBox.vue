<template lang="pug">

        v-card(class="rounded-card")
            div(style="padding:2%")
                v-card-title
                     v-row
                        h4 EE213,Signals and Systems
                        v-flex(class="text-right")
                            v-chip(  class="ma-2 " color="#e3cceb" align="right" )  Submitted on 20th September,2019
                h3 Completed TA Duties

                    div(class="text-center")
                        v-card-text
                            v-row
                                v-textarea(
                                        name="input-7-1"
                                        filled
                                        background-color="white"
                                        label=""
                                        style="border:lightgrey 0.5px solid;"
                                        auto-grow
                                        rounded
                                        value="Tutorial and attendance of EE batch(Tuesday and Thursday)"
                                )
                            v-row
                                v-textarea(
                                    name="input-7-1"
                                    filled
                                    background-color="white"
                                    label=""
                                    style="border:lightgrey 0.5px solid; margin-top: 2%;"
                                    auto-grow
                                    rounded
                                    value="Tutorial and attendance of EE batch(Tuesday and Thursday)"
                                )
                    div(align="right")
                        v-btn(class="ma-2" rounded style="width: 200px;" outlined color="blue" right) Close
</template>

<script>
export default {
    name: "FeedbackTextBox",
};
</script>

